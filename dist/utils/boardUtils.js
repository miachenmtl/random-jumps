...
const getSquareName = (fileIndex, rankIndex) => getFileName(fileIndex) + getRankName(rankIndex);
...
const checkIfLegal = (fileIndex, rankIndex, totalFiles, totalRanks) => checkIfBIsNotOutsideAAndC(0, fileIndex, totalFiles - 1) && checkIfBIsNotOutsideAAndC(0, rankIndex, totalRanks - 1);
...
const getLegalMoves = (fileIndex, rankIndex, totalFiles, totalRanks) => {
  ...
  const {
    ...
    sin,
    round
  } = Math; // each iteration goes out two squares, then sideways plus/minus one square
...
export { getLegalMoves, getSquareName, getRankName, getFileName, parseSquareName, checkIfLegal };