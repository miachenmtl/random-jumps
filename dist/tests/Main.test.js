...
describe("The Main container", () => {
  beforeEach(() => {
    jest.useFakeTimers();
    render( /*#__PURE__*/React.createElement(LangContext.Provider, {
      value: mockContext
  ...
  it("starts a ResizeObserver after mounting and disconnects before unmounting", () => {
    const {
      unmount
    } = render( /*#__PURE__*/React.createElement(Main, null));
  ...
  it("allows the user to show and hide the knight", async () => {
    ...
    const showKnightCheckbox = screen.getByRole("checkbox", {
      name: "Knight"
    });
  ...
  it("allows the user to show and hide the count for each square", async () => {
    ...
    const showCountCheckbox = screen.getByRole("checkbox", {
      name: "Count"
    });
  ...
  it("allows the user to turn heatmap mode on and off for each square", () => {
    ...
    const getWhiteSquareCount = () => {
      const squares = screen.getAllByRole("cell"); // NB: logging the bg color in Square.js gives yields #ffffff

      const whiteSquareCount = squares.reduce((acc, square) => acc + +(getComputedStyle(square).getPropertyValue("background-color") === "rgb(255, 255, 255)"), 0);
  ...
  it("allows the user to drag and drop the initial position of the knight", () => {
    ...
    expect(mockDragEvent.dataTransfer.setData.mock.calls[0][0]).toBe("text/plain");
    ...
    expect(document.querySelector(".drop-target").getAttribute("title")).toBe("c3");
  ...
  it("displays total moves in the stats sidebar", () => {
    ...
    expect(within(leftPanel).getByLabelText("Total moves:")).toHaveTextContent("2");
  });
  it("displays stats for returning to initial square in the sidebar", () => {
    jest.spyOn(boardUtils, "getLegalMoves").mockImplementationOnce(() => [[2, 1]]).mockImplementationOnce(() => [[0, 0]]).mockImplementationOnce(() => [[2, 1]]).mockImplementationOnce(() => [[4, 2]]).mockImplementationOnce(() => [[2, 1]]).mockImplementationOnce(() => [[0, 0]]);
    const startButton = screen.getByText("Start");
    fireEvent.click(startButton);
    jest.advanceTimersByTime(2 * defaultInterval);
    const returnStatsEl = document.querySelector('[data-heading="To initial square"]');
    expect(within(returnStatsEl).getByLabelText("Moves per trip:")).toHaveTextContent("2");
    expect(within(returnStatsEl).getByLabelText("Completed trips:")).toHaveTextContent("1");
    expect(within(returnStatsEl).getByLabelText("Average:")).toHaveTextContent("2");
    jest.advanceTimersByTime(4 * defaultInterval);
    expect(within(returnStatsEl).getByLabelText("Completed trips:")).toHaveTextContent("2");
    expect(within(returnStatsEl).getByLabelText("Moves per trip:")).toHaveTextContent("2, 4");
    expect(within(returnStatsEl).getByLabelText("Average:")).toHaveTextContent("3");
  });
  it("displays stats for visiting all squares in the sidebar", async () => {
    ...
    const newBoardButton = screen.getByText("New Board");
    fireEvent.click(newBoardButton); // we'll cheat and use king moves

    jest.spyOn(boardUtils, "getLegalMoves").mockImplementationOnce(() => [[0, 1]]).mockImplementationOnce(() => [[0, 2]]).mockImplementationOnce(() => [[1, 2]]).mockImplementationOnce(() => [[2, 2]]).mockImplementationOnce(() => [[2, 1]]).mockImplementationOnce(() => [[1, 1]]).mockImplementationOnce(() => [[1, 0]]).mockImplementationOnce(() => [[2, 0]]);
    await waitFor(() => {
    ...
    const tourStatsEl = document.querySelector('[data-heading="To all squares"]');
    expect(within(tourStatsEl).getByLabelText("Completed trips:")).toHaveTextContent("1");
    expect(within(tourStatsEl).getByLabelText("Moves per trip:")).toHaveTextContent("8");
    expect(within(tourStatsEl).getByLabelText("Average:")).toHaveTextContent("8");
  });
});
