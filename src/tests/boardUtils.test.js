...
describe("The getLegalSquares function", () => {
  it("gets the two legal moves from the a1 square", () => {
    ...
    const newMoves = getLegalMoves(0, 0, ...boardDims).map(([x, y]) =>
      getSquareName(x, y)
    );
  ...
  it("gets the eight legal moves from the e4 square", () => {
    ...
    const newMoves = getLegalMoves(4, 3, ...boardDims).map(([x, y]) =>
      getSquareName(x, y)
    );
    const actual = new Set(newMoves);
    expect(actual).toEqual(expected);
  ...
  it("gets the three legal moves from the g8 square", () => {
    ...
    const newMoves = getLegalMoves(6, 7, ...boardDims).map(([x, y]) =>
      getSquareName(x, y)
    );
    const actual = new Set(newMoves);
    expect(actual).toEqual(expected);
  ...
  it("returns pre-calculated moves if called with the same square", () => {
    ...
    const newMoves = getLegalMoves(1, 0, ...boardDims).map(([x, y]) =>
      getSquareName(x, y)
    );
    expect(spy).toHaveBeenCalled();
    const totalCalls = spy.mock.calls.length;
    const newerMoves = getLegalMoves(1, 0, ...boardDims).map(([x, y]) =>
      getSquareName(x, y)
    );
  ...
  it("gets fresh moves if called with the same square but different dimensions", () => {
    ...
    const newMoves = getLegalMoves(6, 6, 8, 8).map(([x, y]) =>
      getSquareName(x, y)
    );
    expect(spy).toHaveBeenCalled();
    const totalCalls = spy.mock.calls.length;

    const newerMoves = getLegalMoves(6, 6, 8, 8).map(([x, y]) =>
      getSquareName(x, y)
    );
    expect(spy).toHaveBeenCalledTimes(totalCalls);
    expect(newerMoves).toEqual(newMoves);

    const newestMoves = getLegalMoves(6, 6, 9, 9).map(([x, y]) =>
      getSquareName(x, y)
    );
